...
:::note[このページのコードについて]
このページは Day 14 時点のコードを解説しています。当時のコードは `git checkout day14` で確認できます。最新のリポジトリには後日の機能追加が含まれます。
:::
...
今日はアプリ本体を起動しなくても確認できます。`app` ディレクトリで、`cluster` パッケージのテストを race detector 付きで実行します。
